...
import memnets.model.impl.DVLikeImpl

import scala.util.Random

private[core] final class SimDImpl(override val system: DynamicSystem, paraThreshold: Int)
    extends SimDImplBase(system, paraThreshold)
class SimDImplBase(override val system: DynamicSystem, paraThreshold: Int) extends SimBase(system) with Logging {
  type T = Double
  import com.github.fommil.netlib.BLAS.{getInstance => blas}
  import memnets.utils.para._
  implicit val locEC = ec
  // need to be careful here
  @inline implicit final def d2T(d: Double): T = d

  val ZERO = 0.0
  val MIN_VALUE = Double.MinValue
  val NEG_INFINITY = Double.NegativeInfinity
  val ONE = 1.0
  val NEG_ONE = -1.0
  val HALF = 0.5
  val LEAP_DECAY = 0.9995
  val TWO = 2.0
  val SIXTH = 1.0 / 6.0
  ...
  object NowTickImpl extends TickImpl {
    ...
    @inline def applyF(i: Int) = _sparseData(i).asInstanceOf[Float]
    ...
    // DenseVectorLike
    def random(lay: LayerLike): Unit = { replace(lay, DenseVector.rand[Double](size = lay.length)) }
    def replace(lay: LayerLike, other: DenseVector[Double]): Unit = {
      require(other.length.toEven == length)
      val xdata = _x(lay.id).data
      System.arraycopy(other.data, 0, xdata, 0, other.data.length) // xdata might be +1 longer
    }
  ...
  protected final class EulerImpl extends IterMethod("Euler") {
    val _k1 = zerosStack
  ...
  protected final class Ode23Impl extends IterMethod("Ode23") {
    val _k1 = zerosStack
    val _k2 = zerosStack
    val iterBuffers = Array(_k1, _k2)
    val _k1Data = data(_k1)
    val _k2Data = data(_k2)
    val _usePara = system.layers.size > 1
  ...
  // NOTE : only here for SimDImpl
  protected final class Ode45Impl extends IterMethod("Ode45") {
    val _k1 = zerosStack
    val k2 = zerosStack
    val k3 = zerosStack
    val k4 = zerosStack
  ...
  protected def weights(link: DenseVectorLike): DV = link.asInstanceOf[DVLikeImpl]._w
  ...
  protected def conv1DCalc(conv: Conv1D) = new LayerLinkCalc {
    ...
    val len = src.length
    val clen = len + klen - 1
    val calc = new Array[Double](clen)
    val zeros = new Array[Double](khalf)
  ...
  protected def customCalc(cus: CustomLink, out: STK, x: STK) = cus.dx(out, x)
  protected def matMulCalc(mat: MatMul, out: STK, x: STK): Unit = {
    import memnets.model.impl.MatMulDImpl
    val a = mat.asInstanceOf[MatMulDImpl]._w
    val b = x(mat.src.id)
  ...
  protected def sparseCalc(spa: SparseLink, out: STK, x: STK): Unit = {
    import memnets.model.impl.SparseLinkDImpl
    val a = spa.asInstanceOf[SparseLinkDImpl]._w
  ...
  protected def stepLambda(lay: LambdaLayer, out: STK, x: STK, f: STPF, ns: NoiseSampler): Unit = {
    ...
    val oData = out(id).data
    val xData = x(id).data
    lay.lambda.dx(oData, xData, f(id, _))
  }
