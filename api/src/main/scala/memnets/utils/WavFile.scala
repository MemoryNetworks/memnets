...
class WavFile private () {
   ...
   * Audio file input stream
    **/
  private var _iStream: InputStream = _

  /**
   * Setter for the iStream field
    **/
  private def iStream_=(value: InputStream): Unit = _iStream = value
  ...
  def readNormalizedFrames(sampleBuffer: Array[Double], numFramesToRead: Int): Int = {
    readNormalizedFrames(sampleBuffer, 0, numFramesToRead)
  }
  ...
  def readNormalizedFrames(sampleBuffer: Array[Double], offset: Int, numFramesToRead: Int): Int = {
    ...
    for (f <- 0 until numFramesToRead) {
      ...
      for (c <- 0 until numChannels) {
        ...
        sampleBuffer(index) = Math.abs(Math.abs(sample) - unsignedToSigned) / maxSignedPCMValue
        index = index + 1
      }
  ...
  def readFrames(sampleBuffer: Array[Int], numFramesToRead: Int): Int = {
    readFrames(sampleBuffer, 0, numFramesToRead)
  }
  ...
  def toArray(): Array[Double] = {
    val numFrames2 = numFrames.toInt
    val readBuffer = new Array[Double](numFrames2 * numChannels)
    readNormalizedFrames(readBuffer, numFramesToRead = numFrames2)
    readBuffer
  }
}
...
object WavFile {
  ...
  class WavFileException(message: String = null, cause: Throwable = null) extends RuntimeException(message, cause)
  ...
  def apply(input: InputStream): WavFile = {
    val wavFile = new WavFile
    wavFile.iStream = input
